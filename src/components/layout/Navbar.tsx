	...
	return (
        ...
        ${scrolled ? 'bg-black/95 backdrop-blur-xl border-b border-[#1e1e1e]' : ''}`}
			>
					...
					{user ? (
						<>
							{isMember ? (
								...
								<Link
									href="/pricing"
									className="clip-btn-sm bg-yellow text-black font-condensed font-bold text-[0.75rem] tracking-[3px] uppercase px-5 py-2.5 no-underline transition-all duration-300 hover:bg-white"
								>
									Upgrade →
								</Link>
							)}
				...
				>
					<span
						className={`block w-6 h-0.5 bg-yellow transition-all duration-300 ${open ? 'rotate-45 translate-y-[7px]' : ''}`}
					/>
					<span
						className={`block w-6 h-0.5 bg-yellow transition-all duration-300 ${open ? 'opacity-0' : ''}`}
					/>
					<span
						className={`block w-6 h-0.5 bg-yellow transition-all duration-300 ${open ? '-rotate-45 -translate-y-[7px]' : ''}`}
					/>
        ...
        ${open ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}
			...
			{showAuth && (
				<AuthModal defaultTab={authMode} onClose={() => setShowAuth(false)} />
			)}
		</>
	);
}
